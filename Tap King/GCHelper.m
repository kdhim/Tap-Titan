...
- (id)init {
    if ((self = [super init])) {
        ...
        if (gameCenterAvailable) {
            ...
            {
                kLeaderBoardIdentifier = @"TapTitan_Phone";
            }
            else
            {
                kLeaderBoardIdentifier = @"TapTitan_Tablet";
            }
        }
    }
    return self;
...
- (void)showLeaderboardOnViewController:(UIViewController*)viewController
{
    ...
    if (gameCenterController != nil) {
        ...
        if ([[UIDevice currentDevice].systemVersion floatValue] < 7.0) {
            gameCenterController.leaderboardCategory = kLeaderBoardIdentifier;
        }
        else {
...
- (bool)isAchievementComplete: (NSString*) identifier
{
    GKAchievement *achievement = [self getAchievementForIdentifier:identifier];
    if (achievement && achievement.percentComplete != 100.0) {
        return false;
